...
The tests should also verify, that standard security annotations work as expected for Quarkus AI extensions
...
Since Quarkus AI may be moved to full support in its first release (Quarkus 3.27), we need to have a full support on all supported platforms (https://access.redhat.com/articles/4966181). This means, that all tests should be in Quarkus Test suite, since running external tests on Openshift will require too much effort.

## Impact on resources
The tests themselves do not consume many resources (single ci.m1.medium/ci.m1.large should be enough).
